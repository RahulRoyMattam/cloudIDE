...
TernaryTree = function() {
    ...
    TernaryTree.prototype = {
        ...
        _all_possible_suffixes : function(node, current_prefix, maxcount,count) {
            var self = this;
            ...
            if (node.right() != null) {
                right_suffixes = self._all_possible_suffixes(node.right(), current_prefix,maxcount);
            } 
            
            if (node.left() != null) {
                left_suffixes = self._all_possible_suffixes(node.left(), current_prefix,maxcount);
            }
            ...
            if (node.center() != null) {
                center_suffixes = self._all_possible_suffixes(node.center(), new_prefix,maxcount);
            }
            ...
            while (node != null) {
                ...
                if (word[position] < node.character)
                    ...
                    if (position == last_character_in_word) { /* end */
                        var suffixes = self._all_possible_suffixes(node.center(), prefix, maxcount,0);
                        if (node.is_word_end)
